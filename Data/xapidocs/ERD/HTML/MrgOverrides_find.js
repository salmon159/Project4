...
function doExp(xxx,yyy){
	if (xxx.style.display=="none"){
		xxx.style.display = ""
		yyy.src = "MrgOverrides_minus.gif";
	}else{
		xxx.style.display = "none"
		yyy.src = "MrgOverrides_plus.gif";
	}
}
	...
	if (searchTokensArray.length > 0)
	{
		...
		if(ArrayLength > 0)
		{
			...
			for ( count = 0; count < ArrayLength; count++)
			{
				...
				strResultsHTML += '<p class="results"><a href="javascript:populateSearchResultDetails(\'results_'+ lastPageID +'_'+ shapeID +'\', '+ lastPageID +','+ shapeID +'); TogglePlus(results_' + lastPageID + '_' + shapeID + ',\'img_' + lastPageID + '_' + shapeID + '\', hideResults)"><img src="MrgOverrides_plus.gif" alt="Shows/hides shape details" width="13" height="9" border="0" id="img_'+ lastPageID +'_'+ shapeID +'"></a>\n'
